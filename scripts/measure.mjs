...
// the old figure. The date line is informational and is not compared — it says
// when the table was last regenerated, which is a different question from
// whether its numbers are currently true. CI answers that one.
...
const lines = [
	...
	'',
	`**Measured on ${stamp}, from i18n-fs ${VERSION}.** Every figure below comes ` +
		'from a real build. CI re-measures on each pull request and fails if this ' +
		'table and the binaries disagree, so a stale number cannot survive here.',
	'',
