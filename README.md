...
**Measured on 2026-08-31, from i18n-fs 0.8.0.** Every figure below comes from a real build. CI re-measures on each pull request and fails if this table and the binaries disagree, so a stale number cannot survive here.

| what | when it is downloaded | gzip | brotli |
| --- | --- | --- | --- |
| **Client Components** — the WebAssembly binary | only when a Client Component calls `useTranslation` | **62.8 KB** | **53.3 KB** |
...
| `browser` | Client Components | downloaded, and only when one is used | 139.0 KB | 62.8 KB | 53.3 KB |
| `node` | Server Components and the CLI | read from disk — never downloaded | 210.1 KB | 100.3 KB | 85.3 KB |
