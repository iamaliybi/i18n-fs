...
**Measured on 2026-08-25, from i18n-fs 0.7.3.** Every figure below comes from a real build. CI re-measures on each pull request and fails if this table and the binaries disagree, so a stale number cannot survive here.
...
| `browser` | Client Components | downloaded, and only when one is used | 122.3 KB | 55.7 KB | 47.8 KB |
| `node` | Server Components and the CLI | read from disk — never downloaded | 194.1 KB | 93.9 KB | 80.1 KB |
