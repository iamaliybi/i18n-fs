...
**Measured on 2026-08-25, from i18n-fs 0.7.2.** Every figure below comes from a real build. CI re-measures on each pull request and fails if this table and the binaries disagree, so a stale number cannot survive here.
...
| `node` | Server Components and the CLI | read from disk — never downloaded | 195.1 KB | 93.9 KB | 80.2 KB |
