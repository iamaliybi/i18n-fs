...
**Measured on 2026-08-24, from i18n-fs 0.5.0.** Every figure below comes from a real build. CI re-measures on each pull request and fails if this table and the binaries disagree, so a stale number cannot survive here.
...
| --- | --- | --- | --- | --- | --- |
| `edge` | the proxy, on every request | server-side — never downloaded | 68.1 KB | 38.3 KB | 33.9 KB |
| `browser` | Client Components | downloaded, and only when one is used | 122.5 KB | 55.7 KB | 47.8 KB |
| `node` | Server Components and the CLI | read from disk — never downloaded | 195.1 KB | 93.9 KB | 80.3 KB |
